 ...
 *
 * @param region - The region code (e.g. "na1", "kr", "euw1").
 * @returns The area name or undefined if not found.
 */
export function getAreaFromRegion(
    region: string
): "americas" | "asia" | "europe" | "sea" | undefined {
    // Region lists grouped by area
    const americas: string[] = ["br1", "la1", "la2", "na1"];
    const asia: string[] = ["jp1", "kr"];
    const europe: string[] = ["euw1", "eun1", "tr1", "ru"];
    const sea: string[] = ["oc1", "ph2", "sg2", "th2", "tw2", "vn2"];
 ...
 *
 * @param summonerPuuid - The PUUID of the summoner to keep in the participants list.
 * @param matchData - The full match data object, typically with an `info.participants` array.
 * @returns A processed match data object containing only the matched summoner's participant data.
 */
